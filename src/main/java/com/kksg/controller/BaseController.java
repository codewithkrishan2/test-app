...
public abstract class BaseController<T extends BaseEntity, R, L, U> {
	...
	public ResponseEntity<ApiResponse<R>> create(@Valid @RequestBody U createRequest) {
		...
		return ResponseEntity
				.ok(new ApiResponse<>(Status.SUCCESS, null, "Saved successfully", mapToResponse(savedEntity)));
	}
...
//    }
	...
	@GetMapping
	public ResponseEntity<ApiResponse<List<L>>> getAll() {
		List<T> entities = baseService.findAll();
		List<L> responseList = entities.stream().map(this::mapToListResponse).toList();
		return ResponseEntity.ok(new ApiResponse<>(Status.SUCCESS, null, "Fetched successfully", responseList));
	}
	...
	public ResponseEntity<ApiResponse<R>> getById(@PathVariable Long id) {
		return baseService.findById(id)
				.map(entity -> ResponseEntity
						.ok(new ApiResponse<>(Status.SUCCESS, null, "Fetched successfully", mapToResponse(entity))))
	...
	public ResponseEntity<ApiResponse<R>> update(@PathVariable Long id, @Valid @RequestBody U updateRequest) {
		return baseService.findById(id).map(existingEntity -> {
			T updatedEntity = mapToEntity(updateRequest);
			updatedEntity.setId(id);
			T savedEntity = baseService.save(updatedEntity);
			return ResponseEntity
					.ok(new ApiResponse<>(Status.SUCCESS, null, "Updated successfully", mapToResponse(savedEntity)));
		}).orElse(ResponseEntity.status(404).body(new ApiResponse<>(Status.FAILED, notFoundMessage, null, null)));
	}

	@DeleteMapping("/deleteFlagData/{id}")
	public ResponseEntity<ApiResponse<Void>> delete(@PathVariable Long id) {
		return baseService.findById(id).map(entity -> {
	...
	protected abstract L mapToListResponse(T entity);
}